...
* `alanfischer/goldsrc-godot` runtime availability;
...
3. Install or build `alanfischer/goldsrc-godot` locally so Godot can see
   `GoldSrcMDL` and `GoldSrcSPR`. The adapter expects the GDExtension classes
   documented by that repository: `GoldSrcMDL`, `GoldSrcSPR`, `scale_factor`,
   `load_mdl()`, `build_model()`, sequence timing and bone/bodypart/skin
   inspection methods.

This repository does not commit addon binaries in PR-06. If the addon is
missing, the tool reports `goldsrc_godot_extension_missing` instead of using a
placeholder or fake mesh.
...
Expected without the addon installed:
...
After installing `goldsrc-godot` and configuring `user://local_goldsrc.json`,
run:
