...
const OpenStrikeCvarRegistryRef = preload("res://src/core/config/cvar_registry.gd")


static func load_default_cvars(path: String = DEFAULT_CVARS_PATH):
	var registry = OpenStrikeCvarRegistryRef.new()
	registry.load_cfg_file(path)
	return registry


static func create_empty_bind_registry():
	return OpenStrikeBindRegistryRef.new()
